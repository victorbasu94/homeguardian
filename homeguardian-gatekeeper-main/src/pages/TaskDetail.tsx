    ...
    const fetchTask = async () => {
      ...
      try {
        ...
        if (import.meta.env.PROD) {
          ...
          // Set the task data based on the API response
          if (data && data.id) {
            setTask(data);
          } else if (data && data.task && data.task.id) {
            setTask(data.task);
            ...
            if (foundTask) {
              console.log('Task found in tasks array:', foundTask);
              setTask(foundTask);
            } else {
        ...
        // In development, fall back to mock data even if there's an error
        if (!import.meta.env.PROD) {
